...
class RealSense(VideoSrc):
    ...
    def _get_frames(self):
        ...
        resized_depth_image = cv2.resize(depth_image, (self.width, self.height), interpolation=cv2.INTER_LINEAR)
    ...
    def depth2colormap(self, depth):
        ...
        if non_zero_values.size == 0:
            return np.zeros_like(modified_array, dtype=np.uint8)  # Return zeros if all values are below threshold

        min_val = np.min(non_zero_values)
        max_val = np.max(non_zero_values)

        # Scale the remaining values to the range [0, 255]
        scaled_array = np.zeros_like(modified_array, dtype=np.float64) # Initialize with zeros
        scaled_array[modified_array > 0] = ((non_zero_values - min_val) / (max_val - min_val)) * 255
